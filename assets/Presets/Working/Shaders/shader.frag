...
uniform vec3 pos; 			//ui:-1.0,1.0,1.0
uniform vec3 light; 			//ui:-10.0,10.0,1.0
uniform float radius; 		//slider:0.0,2.0,1.0
uniform float shadowScale; 	//slider:1.0,4.0,2.0
uniform int speed; 		//slider:1.0,16.0,2.0
uniform vec2 amount;		//range:0.0,1.0,0.25,0.75
uniform int freq; 			//slider:1.0,16.0,2.0
uniform float cameraDistance; //slider:1.0,5.0,3.5

in vec4 vColor;
in vec2 vTexcoord;

float bounce = sin( iAnimationTime * PI );
vec4 sph1 = vec4( pos + vec3( 0.0, 0.0 * bounce, 0.0 ), radius );
...
{
	vec2 uv = vTexcoord;
	vec3 ro = vec3( 0.0, 1.0, cameraDistance );
	...
	if( id > 0.5 && id < 1.5 ) {
		...
		float dir = nor.x / ( nor.y + 1.0 ) + 2.0 * nor.z ;

		float value = abs( sin( float( freq ) * dir * TWO_PI + float(speed) * iAnimationTime * TWO_PI ) );
		if( value > amount.x && value < amount.y )
		{
			col *= 0.0;
		}

		value = abs( sin( float( freq ) * dir * TWO_PI + float(speed) * iAnimationTime * TWO_PI ) );
		if( value > amount.x && value < amount.y ) {
			col *= 0.0;
		}
	}
