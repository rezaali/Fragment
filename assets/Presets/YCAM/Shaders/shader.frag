...
void main(void)
{
    ...
    if( iAnimationTime < 0.5 ) {
        time = smoothstep( 0.0, 1.0, iAnimationTime / 0.5 );
    }
    else {
        time = smoothstep( 0.0, 1.0, 1.0 - ( iAnimationTime - 0.5 ) / 0.5 );
    }

    float nv = 1.0 + snoise( vec3( 3.0 * p, time ) );
    nv /= 2.0;
    col *= sin( nv * 100.0 ) > 0.25 ? 1.0 : 0.0;
